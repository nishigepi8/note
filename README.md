# 技术笔记与博客文章

本仓库是我个人技术笔记和博客文章的备份，按照技术领域分类整理。

## 目录结构

```
note/
├── 音视频/          # 音视频技术：RTP、P2P、SFU/MCU 架构
├── K8s/             # Kubernetes 容器编排与运维
├── 物联网/          # 物联网：Matter 协议、MQTT 安全架构
├── 开发规范/        # 开发规范：Git 分支、CI/CD
├── 架构设计/        # 架构设计：授权机制、K8s 架构图
├── 项目实践/        # 项目实践：视频传输、嵌入式开发
├── 开发工具/        # 开发工具：效率提升、工具链配置
└── ABOUT.md         # 关于我
```

## 文章列表

### 🎥 音视频技术

| 文章 | 简介 |
|------|------|
| [P2P、SFU和MCU音视频通信架构](/post/音视频/P2P、SFU和MCU音视频通信架构.md) | 深入对比三种音视频架构的优缺点和适用场景 |
| [RTP 实时传输协议](/post/音视频/RTP%20实时传输协议.md) | RTP 协议深度解析，包括协议头、时间戳同步、性能优化 |
| [WebRTC 信令服务详解](/post/音视频/WebRTC%20信令服务详解.md) | Offer/Answer/ICE Candidate 详解，含代码示例和调试技巧 |
| [Luckfox Pico Max P2P直播方案](/post/音视频/Luckfox%20Pico%20Max%20P2P直播方案.md) | 低成本嵌入式 P2P 视频直播，C + Golang 混合架构 |

### ☸️ Kubernetes 容器编排

| 文章 | 简介 |
|------|------|
| [Kubernetes 常用命令实战指南](/post/K8s/Kubernetes%20常用命令实战指南.md) | Kubernetes 日常运维命令实战指南 |
| [Kubernetes 滚动更新实战指南](/post/K8s/Kubernetes%20滚动更新实战指南.md) | 滚动更新策略、配置和最佳实践 |

### 📡 物联网

| 资源 | 简介 |
|------|------|
| Matter 协议规范 | 包含 Matter 1.2 设备库规范、应用集群规范 |
| 物联网 MQTTS 安全架构 | MQTT over TLS 安全传输架构设计 |

### 🔧 开发规范

| 文章 | 简介 |
|------|------|
| [Git 分支与 Commit 规范](/post/开发规范/Git%20分支与%20Commit%20规范.md) | 中大型团队 GitFlow 实践，Conventional Commits 规范 |

### 🏗️ 架构设计

| 文章 | 简介 |
|------|------|
| [物模型：IoT 设备标准化实践](/post/架构设计/物模型：IoT%20设备标准化实践.md) | 从硬件到能力的抽象思想，物联网设备标准化开发的设计理念与工程实践 |
| [图片向量存储与相似性搜索方案](/post/架构设计/图片向量存储与相似性搜索方案.md) | 基于 CLIP + Milvus 的图片向量存储与相似性搜索，包含 AI 处理、颜色打标、类型分类 |
| [高并发缓存同步 RSC方案](/post/架构设计/高并发缓存同步%20RSC方案.md) | 借鉴 JVM Survivor 机制，组合 Redis+Kafka+MongoDB 实现百万设备状态同步 |
| [Kafka Partition 规划与问题处理](/post/架构设计/Kafka%20Partition%20规划与问题处理.md) | Partition 数量规划、消费延迟、Rebalance 等常见问题的处理方案 |
| [GPS 轨迹存储方案分析](/post/架构设计/GPS%20轨迹存储方案分析.md) | PostGIS、MongoDB、Redis 方案对比，含性能优化实践 |
| [自研 P2P 服务架构设计](/post/架构设计/自研%20P2P%20服务架构设计.md) | 基于 Pion 的 STUN/TURN/信令服务，支持 10 万并发 |
| [OLAP数据库选型对比](/post/架构设计/OLAP数据库选型对比.md) | StarRocks、ClickHouse、InfluxDB 三大数据库架构、性能、场景全面对比，含选型决策树 |
| Auth-Service 授权机制.pdf | 微服务授权架构设计 |
| BMGuardr Kubernetes 架构.pdf | K8s 集群架构设计方案 |
| CI 架构图.pdf | 持续集成流水线架构 |

### 💡 项目实践

| 文章 | 简介 |
|------|------|
| [AI 生成训练数据增强 YOLO 检测方案](/post/项目实践/AI生成训练数据增强YOLO检测方案.md) | 用 AI 训练 AI：使用 Stable Diffusion 生成训练数据，混合训练策略避免数据偏差 |
| [OpenResty + Redis 短链接服务系统](/post/项目实践/OpenResty%20+%20Redis%20短链接服务系统.md) | 使用 OpenResty + Redis 构建完整业务系统，单机 10 万+ QPS，延迟 < 5ms |
| [Python MQTT 实时视频传输](/post/项目实践/Python%20MQTT%20实时视频传输.md) | MQTT 用于视频传输的实践、性能优化与局限性分析 |

### 🛠️ 开发工具

| 文章 | 简介 |
|------|------|
| [Mac 开发效率提升指南](/post/开发工具/Mac%20开发效率提升指南.md) | Alfred、iTerm2、Alias 系统，提效 10%+ |

## 技术栈

- **后端**：Java、Golang、Python
- **物联网**：Matter、MQTT、嵌入式 Linux
- **音视频**：WebRTC、RTP、H.264
- **运维**：Kubernetes、Docker、CI/CD
- **硬件**：PCB 设计、Luckfox、RV1106

## 更新日志

### 写作风格的演进

- **2018-2024**：从 2018 年开始记录技术笔记，更多关注"怎么做"（How），记录各种技术的具体实现方法和操作步骤
- **2024 至今**：写作重心逐渐转向"为什么"（Why）和"是什么"（What），更关注设计理念、架构思想、技术本质的探讨

### 最近更新

- 2025-12-25：新增《物模型：IoT 设备标准化实践》，从设计理念到工程实践的完整探讨
- 2025-12-24：文件夹名称改为中文，更直观
- 2025-12-24：重构目录结构，按技术领域分类整理
- 2025-12-21：初始迁移，优化现有文档格式和内容
- 2025-12-20：博客开源 [PowerWiki](https://github.com/steven-ld/PowerWiki.git)

## 关于

这些文章最初发布在 [ga666666.cn](https://ga666666.cn)，现在迁移到本地 Markdown 格式，便于版本控制和持续优化。

更多关于我的信息，请查看 [ABOUT.md](./ABOUT.md)。

如有问题或建议，欢迎交流讨论！
