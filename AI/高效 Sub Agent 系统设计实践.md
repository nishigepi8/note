...
为了让这套系统不只是“看起来高级”，我后来又单独整理了一个对比目录，把**单 Agent** 和 **多 Agent** 在同一个任务上的输出放在一起看。

- [案例对比目录](./高效 Sub Agent 系统设计实践-案例对比/README.md)
- [单 Agent 输出](./高效 Sub Agent 系统设计实践-案例对比/single-agent-output.md)
- [多 Agent 输出](./高效 Sub Agent 系统设计实践-案例对比/multi-agent-output.md)

这个对比很直观：
- 单 Agent 容易把分析、方案、风险、总结混在一起
- 多 Agent 能把职责拆清楚，并通过依赖和消息流把结果收敛起来
